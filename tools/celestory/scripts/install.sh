#!/bin/sh
# celestory installer (macOS / Linux)
#
# Downloads the latest celestory release binary for your OS/arch, verifies its
...
#
# Re-run to upgrade in place. Override the target dir with CELESTORY_INSTALL_DIR.
set -eu

REPO="sidthesloth92/db-astro-suite"
COMPONENT="celestory"
BINDIR="${CELESTORY_INSTALL_DIR:-$HOME/.local/bin}"

err() { printf 'celestory: %s\n' "$1" >&2; exit 1; }
info() { printf '%s\n' "$1"; }
...
# --- find the newest celestory release --------------------------------------
info "Finding the latest $COMPONENT release..."
# The repo is a monorepo, so releases/latest isn't celestory-specific. List
...
if [ -n "$existing" ]; then
  ...
  if [ -n "$cur" ] && [ "$cur" = "$version" ]; then
    info "celestory $version is already installed ($existing) — nothing to do."
    exit 0
  fi
fi
...
tmp=$(mktemp -d 2>/dev/null || mktemp -d -t celestory)
...
if [ -n "$cur" ]; then
  info "Upgraded celestory $cur -> $version."
else
  info "Installed celestory $version to $dest."
fi
...
info "Run 'celestory' to get started."
