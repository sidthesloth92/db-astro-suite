...
<#
.SYNOPSIS
    Installs the latest Celestory CLI for Windows from GitHub Releases.

.DESCRIPTION
    Downloads the latest celestory_windows_<arch>.zip from the
...
$Component  = 'celestory'
$InstallDir = Join-Path $env:LOCALAPPDATA 'Programs\Celestory'
$Headers    = @{ 'User-Agent' = 'celestory-installer' }
...
# celestory-specific. List releases and pick the newest whose tag is prefixed
# with the component name (release-please tags as 'celestory-vX.Y.Z').
...
$tmp = Join-Path $env:TEMP ("celestory-" + [guid]::NewGuid().ToString() + ".zip")
...
Write-Host ""
Write-Host "Installed celestory $($release.tag_name) to $InstallDir"
Write-Host "Run 'celestory' to get started."
