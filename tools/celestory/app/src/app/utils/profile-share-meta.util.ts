 ...
 * target or equipment named by the `?target=` / `?equipment=` query param when
...
/** Which target / equipment a profile link is focused on (from the URL query). */
...
export function profileShareMeta(
  ...
  if (focus.target) {
    const o = story.targets.find((x) => x.id === focus.target);
    if (o) {
      ...
      const name =
        o.designation && o.designation !== o.displayName
          ...
          : o.displayName;
      const q = `?target=${encodeURIComponent(focus.target)}`;
      return {
  ...
  if (focus.equipment) {
    ...
    if (e) {
      ...
      return {
        ...
        description: `${e.kind} · ${hours}h · ${formatCount(e.targetCount)} targets · ${formatCount(e.lightFrameCount)} light frames.`,
        type: 'article',
        url: abs(origin, `/user/${handle}${q}`),
        image: abs(origin, `/api/og/user/${handle}${q}`),
      };
  ...
  return {
    ...
    description: `${formatCount(summary.targetCount)} targets · ${hours}h integration · ${formatCount(summary.nightCount)} nights imaged.`,
    type: 'profile',
