...
func TestOpenMasksHomePaths(t *testing.T) {
	...
	sess.Logger.Info("scan start",
		"file", filepath.Join(home, "captures", "m31.fit"),
		"other", "/Volumes/T7/captures/m31.fit")
	if err := sess.Close(); err != nil {
		t.Fatalf("close: %v", err)
	}
	...
	if !strings.Contains(out, "~"+sep+"captures"+sep+"m31.fit") {
		t.Errorf("log missing the masked path:\n%s", out)
	}
	if !strings.Contains(out, "/Volumes/T7/captures/m31.fit") {
...
// TestOpenRotatesOversizedLog exercises rotation through the real open path
// (not the helper): an oversized active log is moved to the .1 backup and a
// fresh session starts a new, small active file.
func TestOpenRotatesOversizedLog(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("expected the oversized log rotated to %s.1: %v", logFileName, err)
	}
	if backup.Size() != maxLogBytes+1 {
		t.Errorf("backup size = %d, want %d", backup.Size(), maxLogBytes+1)
	}
	active, err := os.Stat(path)
	if err != nil {
		t.Fatalf("expected a fresh active log: %v", err)
	}
	if active.Size() >= maxLogBytes {
		t.Errorf("active log should have restarted small, size = %d", active.Size())
	}
}
