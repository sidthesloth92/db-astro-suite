...
export default async function (fastify, opts) {
  ...
  fastify.post('/api/v1/solve', async (request, reply) => {
    // We expect the image to be attached as `image` and other fields optionally.
    ...
    if (!hints.pixel_size) {
      // It's technically safe to attempt without if we do blind solves, but requirements said "required string/number"
