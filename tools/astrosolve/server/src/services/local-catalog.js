...
try {
  ...
  console.error("Local Celestial Database not found. Run 'node src/scripts/seed_db.mjs' first.");
}
 ...
 */
export function queryLocalCatalog({ra, dec, radiusDeg, maxMagnitude = 10, types = []}) {
  if (!db) return [];
  ...
  let sql = `
    SELECT catalog, entryId, name, commonName, type, ra, dec, magnitude
    FROM objects
  ...
  const results = candidates.filter(obj => {
