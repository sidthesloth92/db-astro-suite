...
| `hub_launch_tool_clicked` | `tool` (`starwizz` \| `astrogram` \| `file-grouper`), `destination` (the launch href)   | User clicks "Launch Tool" / "Access Repository" on a dossier page — this is the **hub → tool conversion event** | [astrogram.page.ts](hub/src/app/pages/dossier/astrogram.page.ts), [starwizz.page.ts](hub/src/app/pages/dossier/starwizz.page.ts), [file-grouper.page.ts](hub/src/app/pages/dossier/file-grouper.page.ts) |

The two events form a simple funnel: home → dossier (`hub_tool_card_clicked`) → tool launch (`hub_launch_tool_clicked`). The file-grouper card on the home page is disabled and has no routerLink, so its card click is not tracked — but its dossier page's "Access Repository" link is.
