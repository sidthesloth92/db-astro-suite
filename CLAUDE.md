# Claude Code Instructions — db-astro-suite
...
Path-specific rule files in `.claude/rules/` provide detailed per-stack rules — load the one matching the path you are editing.
...
## Project Subagents & Skills

This project ships custom subagents and skills under `.claude/`:

- **`.claude/agents/`** — `feature-agent` (orchestrator), `frontend-dev`, `backend-dev`, `lead-pr-reviewer`, `e2e-tester`, `infra-engineer`. See `.claude/agents/README.md` for invocation and the new-feature / PR-feedback flows.
- **`.claude/skills/`** — `angular-component`, `backend-api`, `playwright-e2e`, `pr-review-checklist`, `create-pr`, `pipeline-ops`, `pr-feedback`. Agents load these on demand; you can also invoke a skill directly via the Skill tool.

When orchestrating a feature end-to-end, route through `feature-agent`. For single-stack tweaks, invoke the matching dev agent directly.
