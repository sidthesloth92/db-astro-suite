 ...
 * a themed blue→pink gradient, scattered twinkle stars (opacity 0), a faint
 * orbit, the edges (fully dash-offset so they read as undrawn) and the nodes
 * (scaled to 0). The component injects this once — in the browser only — then
 * animates the elements in via the Web Animations API.
 *
 * Themed colours are written as inline `style` so `var(--db-color-*)` resolves
 * (SVG presentation attributes don't). Elements are tagged with `cl-line`,
 * `cl-node`, `cl-ring` and `cl-star` classes so the component can drive them.
 *
...
export function buildConstellationMarkup(gradientId: string): string {
  ...
  let out =
    ...
    `<stop offset="0%" style="stop-color:${c.gradientFrom}"/>` +
    `<stop offset="100%" style="stop-color:${c.gradientTo}"/></linearGradient></defs>`;
  ...
  for (let i = 0; i < CL_STAR_COUNT; i++) {
    ...
    out += `<circle class="cl-star" cx="${x}" cy="${y}" r="${r}" fill="${c.star}" opacity="0"/>`;
  }

  // Faint outer orbit (themed gradient).
  out +=
  ...
  CL_NODES.forEach((point, i) => {
    ...
    const inner = CL_BIG.includes(i)
      ? `<path d="${sparklePath(x, y, 5.4)}" fill="${c.star}"/>` +
        `<circle class="cl-ring" cx="${x}" cy="${y}" r="8.6" fill="none" ` +
        `style="stroke:${c.reticle}" stroke-width="2"/>`
      : `<circle cx="${x}" cy="${y}" r="3" fill="${c.star}"/>` +
        `<circle cx="${x}" cy="${y}" r="6" style="fill:${c.halo}" opacity="0.22"/>`;
    out +=
